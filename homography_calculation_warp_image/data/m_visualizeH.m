function fig = m_visualizeH(i1,i2,H)
% fig = m_visualizeH(i1,i2,H)
% Visualizes an homography matrix of two views
% By: Minh Hoai Nguyen (minhhoai@cs.stonybrook.edu)
% Created: 21-Oct-2016
% Last modified: 21-Oct-2016
% Adapted from vgg_gui_H of AZ
...
if strcmp(action,'start'),
   ...
   h0 = figure('Color',[0.8 0.8 0.8], ...
	       ...
	       'WindowButtonUpFcn', 'm_visualizeH(''none'');',...
	       'WindowButtonMotionFcn', 'm_visualizeH(''move'')', ...
	       'Pointer', 'crosshair', ...
	       'DoubleBuffer', 'on',...
	       'Units','normalized');
   m0=uimenu('Label', '&Color');
   uimenu(m0, 'Label', '&Red', 'ForegroundColor', [1 0 0], ...
	  'Accelerator', 'r', 'Callback', 'm_visualizeH(''cr'');');
   uimenu(m0, 'Label', '&Green', 'ForegroundColor', [0 1 0], ...
	  'Accelerator', 'g', 'Callback', 'm_visualizeH(''cg'');');
   uimenu(m0, 'Label', '&Blue', 'ForegroundColor', [0 0 1], ...
	  'Accelerator', 'b', 'Callback', 'm_visualizeH(''cb'');');
   m1=uimenu('Label', '&Size');
   uimenu(m1, 'Label', '&Increase', 'Callback', 'm_visualizeH(''s+'');', 'Accelerator', '+');
   uimenu(m1, 'Label', '&Decrease', 'Callback', 'm_visualizeH(''s-'');', 'Accelerator', '-');
   uimenu(m1, 'Label', '&1', 'Callback', 'm_visualizeH(''s1'');', 'Accelerator', '1');
   uimenu(m1, 'Label', '&2', 'Callback', 'm_visualizeH(''s2'');', 'Accelerator', '2');
   uimenu(m1, 'Label', '&3', 'Callback', 'm_visualizeH(''s3'');', 'Accelerator', '3');
   uimenu(m1, 'Label', '&4', 'Callback', 'm_visualizeH(''s4'');', 'Accelerator', '4');
   uimenu(m1, 'Label', '&5', 'Callback', 'm_visualizeH(''s5'');', 'Accelerator', '5');
   uimenu(m1, 'Label', '&6', 'Callback', 'm_visualizeH(''s6'');', 'Accelerator', '6');
   uimenu(m1, 'Label', '&7', 'Callback', 'm_visualizeH(''s7'');', 'Accelerator', '7');
   uimenu(m1, 'Label', '&8', 'Callback', 'm_visualizeH(''s8'');', 'Accelerator', '8');
   uimenu(m1, 'Label', '&9', 'Callback', 'm_visualizeH(''s9'');', 'Accelerator', '9');
   
   ah1 = axes('Parent', h0, ...
	      'Position',[0 0 .5 1]);
   h1=imshow(i1); hold on; title('Image 1');
   set(h1, 'ButtonDownFcn','m_visualizeH(''b1'');');

   ah2 = axes('Parent',h0, ...
	      'Position',[.5 0 .5 1], ...
	      'Tag','Axes2');
   h2=imshow(i2); hold on; title('Image 2');
   set(h2, 'ButtonDownFcn','m_visualizeH(''b2'');');
